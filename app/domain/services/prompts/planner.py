...
- 如果 attachments 非空，它们就是用户已上传且已经同步到沙箱中的文件路径。
- 当用户要求“读取文件/读取附件/查看上传文件内容/这个文件内容”但没有在消息中写明路径时，必须使用 attachments 中的文件路径来规划读取操作，不要反问用户提供路径。
...
输入:
- message: 用户的消息
- attachments: 用户的附件；每一行是沙箱内可直接使用的文件路径
