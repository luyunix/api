...
- 如果 attachments 非空，它们就是用户已上传且已经同步到沙箱中的文件路径，可以直接传给文件工具或 shell 工具使用。
- 当用户要求读取文件/附件/上传文件内容，但当前步骤没有写明具体路径时，必须优先使用 attachments 中的文件路径，不要反问用户提供路径。
- 如果只有一个附件且用户提到“文件/附件/上传的文件”，默认该附件就是用户指代的文件。
...
- attachments: 用户提供的附件；每一行是沙箱内可直接使用的文件路径
- language: 当前的工作语言
- task: 当前需要执行的任务
